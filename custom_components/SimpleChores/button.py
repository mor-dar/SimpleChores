...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, add_entities: AddEntitiesCallback):
    ...
    for reward in coordinator.get_rewards():
        for kid_id in coordinator.model.kids.keys():
...
class SimpleChoresCreateChoreButton(ButtonEntity):
    ...
    async def async_press(self) -> None:
        ...
        title_entity = self._hass.states.get(f"text.{DOMAIN}_chore_title_input")
        points_entity = self._hass.states.get(f"text.{DOMAIN}_chore_points_input") 
        kid_entity = self._hass.states.get(f"text.{DOMAIN}_chore_kid_input")
